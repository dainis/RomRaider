 ...
 * field values.
 */
public final class VntVoltagePluginMenuAction extends AbstractAction {
    private static final String PHIDGET_VNT = "Phidget VNT";
     ...
     */
    public VntVoltagePluginMenuAction(final EcuLogger logger) {
        super(logger);
    }

    @Override
    public void actionPerformed(ActionEvent actionEvent) {
        final VntVoltageConvertorPanel intfKitPanel =
                new VntVoltageConvertorPanel(logger, getDataItems());
    ...
    private List<List<String>> getDataItems() {
        ...
        for (ExternalDataSource source : externalSources) {
            if (source.getName().equals(PHIDGET_VNT)) {
    ...
    private final void saveChanges(JTable table) {
        ...
        final Map<String, VntVoltageSensor> phidgets = SettingsManager.getSettings().getPhidgetVntVoltageSensors();

        for (int i = 0; i < tm.getRowCount(); i++) {
            String column0 = (String) tm.getValueAt(i, 0);
            String key = column0.replaceAll("Phidget VNT Sensor ", "");
            ...
            final VntVoltageDataItem ikDataItem = (VntVoltageDataItem) dataItems.get(i);
